...
def predict_alpha(model, mesh, filename):
    ...
    with torch.no_grad():
        for i_sample in range(1): #sample_filenames:
            ...
            output_file_name_d_refined = '{}_d_predicted_refined'.format(filename[:-4])
            vedo.write(mesh3, os.path.join(output_path, output_file_name_d_refined + ".vtp"))
            ...
            output_file_name_refined = '{}_predicted_refined'.format(filename[:-4])
            vedo.write(mesh, os.path.join(output_path, output_file_name_refined + ".vtp"))
